...
@Component({
    selector: 'siniestro-otros',
    templateUrl: 'siniestroOtros.html'
})

export class SiniestroOtros implements OnInit {

    siniestroOtrosForm:FormGroup;
    siniestroOtrosDocumentationForm:FormGroup;
    ...
    constructor(public mapsApiLoader:MapsAPILoader ,public ngZone:NgZone  ,public http:Http,public local:UserSessionService,public formBuilder:FormBuilder, public router:Router , public select:SelectService) { 
        ...
        this.siniestroOtrosForm = this.formBuilder.group({
            dataPatient:[],
            sinisterDiagnosis:[],
            sinisterValue:[],
            deductibleValue:[],
    ...
    saveSiniestro(){
        this.saved.emit({form: this.siniestroOtrosForm.value, items: this.docSiniestroRamos });
    }
