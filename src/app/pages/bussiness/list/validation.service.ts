export class ValidationScheduleService {
    ...
    static matchingDays(startKey: string, endKey: string) {
        return (group) => {
            let date_start= group.controls[startKey];
            let date_end = group.controls[endKey];
            if (date_end.value > date_start.value) {
                return date_end.setErrors({invalidDayEnd: true})
            }
        }
    }
    static matchingHours(startKey: string, endKey: string) {
        return (group) => {
            let start= group.controls[startKey];
            let end = group.controls[endKey];
            if (end.value > start.value) {
                return end.setErrors({invalidHourEnd: true})
            }
        }
    }
