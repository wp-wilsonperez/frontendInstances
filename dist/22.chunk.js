...
var ValidationService = (function () {
    ...
    ValidationService.rucValidator = function (control) {
    ...
    ValidationService.validacionCedula = function (cedulaKey) {
        return function (group) {
...
var LoginComponent = (function () {
    function LoginComponent(router, fb, http, local) {
        this.http = http;
        this.local = local;
        this.loginErr = false;
        this.recover = false;
        this.router = router;
        this.form = fb.group({
            'cedula': ['', __WEBPACK_IMPORTED_MODULE_6__angular_forms__["Validators"].compose([__WEBPACK_IMPORTED_MODULE_6__angular_forms__["Validators"].required])],
            'password': ['', __WEBPACK_IMPORTED_MODULE_6__angular_forms__["Validators"].compose([__WEBPACK_IMPORTED_MODULE_6__angular_forms__["Validators"].required, __WEBPACK_IMPORTED_MODULE_6__angular_forms__["Validators"].minLength(6)])]
        }, { validator: __WEBPACK_IMPORTED_MODULE_3__user_new_validation_service__["a" /* ValidationService */].validacionCedula('cedula') });
    ...
    LoginComponent.prototype.onSubmit = function (values) {
        var _this = this;
        if (this.form.valid) {
            this.http.post(__WEBPACK_IMPORTED_MODULE_2__config_project_config__["a" /* config */].url + 'user/login', values)
                .toPromise().then(function (result) {
                var apiResult = result.json();
                console.log(apiResult);
                if (apiResult.login) {
                    var id = apiResult.user.idRol;
                    var token = apiResult.user.token;
                    _this.user = apiResult.user;
                    var rolResult = result.json();
                    Object.assign(_this.user, rolResult);
                    _this.local.setUser(_this.user);
    ...
    LoginComponent.prototype.showRecover = function (val) {
        this.recover = val;
    };
...
// validacion de cedula
function validacionCedula(cedulaKey) {
    return function (group) {
        var cedula = group.controls[cedulaKey];
        var d1 = cedula.value.substr(0, 1);
        var d2 = cedula.value.substr(1, 1);
